...
def VonMisesFisherLoss(pred: Tensor, truth: Tensor, kappa_switch: float = 100., reg_factor: float = 0.0, eps: float = 1e-6) -> Tensor:
    ...
    Args:
        pred: Predicted unnormalized direction vectors [B, D], where ||pred|| = kappa.
    ...
    kappa = torch.norm(pred, dim=1).clamp(min=eps) # Calculate kappa = ||pred||, ensure > 0
    dotprod = torch.sum(F.normalize(pred, dim=1) * truth, dim=1) # Cosine similarity between directions
