...
class MmapDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        for path in mmap_paths:
            ...
            if self.dataset_type is None:
                ...
                elif 'BDT_pred' in field_names:
                    self.dataset_type = 'magnemite'
                else:
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_type == 'prometheus':
            ...
            hadrons_pos_x = event_record['final_x'][1]
            hadrons_pos_y = event_record['final_y'][1]
            hadrons_pos_z = event_record['final_z'][1]
            gamma_pos_x = event_record['final_x'][2]
            gamma_pos_y = event_record['final_y'][2]
            gamma_pos_z = event_record['final_z'][2]
            labels = np.array([log_energy, dir_x, dir_y, dir_z, class_label, hnl_length, hadrons_energy, gamma_energy], dtype=np.float32)
        else:
